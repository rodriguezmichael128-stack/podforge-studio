...
function buildAdminAnalytics() {
  ...
  const completedCollections = collections.filter((collection) => ["Collected", "Overdue"].includes(collection.status));
  const collectionSuccessRate = completedCollections.length
    ? Math.round((completedCollections.filter((collection) => collection.status === "Collected").length / completedCollections.length) * 100)
    : 0;

  return {
    generatedAt: new Date().toISOString(),
    mode: !isStripeConfigured() ? "unavailable" : isLiveStripeKey() ? "live" : "test",
    ...
    summary: {
      ...
      connectedPlatforms: platforms.filter((platform) => ["Connected", "Native"].includes(platform.status)).length,
...
async function createMandateSetup(payload) {
  ...
  if (!isStripeConfigured()) {
    return {
      ...
      stripeSessionId: "",
      billingMethod,
      message: "Stripe is not configured yet, so no billing mandate was created.",
    };
  }
  ...
  const customer =
    ...
    (
      await stripeRequest("POST", "/v1/customers", {
        ...
        metadata: {
          podforge_creator_id: payload.creatorId || "",
        },
      })
    ).id;
...
async function collectPlatformFee(payload) {
  ...
  if (!isStripeConfigured()) {
    return {
      ...
      fee: 0,
      message: "Stripe is not configured yet, so no fee was collected.",
    };
  }
...
async function handleApi(request, response, url) {
  ...
  if (request.method === "POST" && url.pathname === "/api/revenue/platform-sync") {
    ...
    sendJson(response, 200, {
      ...
      importedRows: 0,
      message: "Platform revenue sync is not connected yet, so no rows were imported.",
    });
    return true;
  }
